	...
	for i in range(len(postags)):
		if postags[i] in usefulWord:
			...
			if(len(words[i]) == 3 and postags[i] == 'v'):
				...
				try:
					if(cur.execute("select KeyWord from a%s where KeyWord = '%s' and type='keyword'" % (ID,words[i])) == 0):
						cur.execute("insert into a%s (KeyWord,num,%s,type)values ('%s',1,1,'keyword')" % (ID,month[time],words[i]))
						conn.commit()
					else:
						cur.execute("update a%s set num = num + 1 where KeyWord = '%s' and type='keyword'" % (ID,words[i]))
						conn.commit()
						cur.execute("update a%s set %s = %s + 1 where KeyWord = '%s' and type='keyword'" % (ID,month[time],month[time],words[i]))
						conn.commit()
				except MySQLdb.Error,e:
					continue
	##平台统计
	try:
		if(cur.execute("select KeyWord from a%s where KeyWord = '%s' and type='tool'" % (ID,tool)) == 0):
			cur.execute("insert into a%s (KeyWord,type,num)values ('%s','tool',1)" % (ID,tool))
			conn.commit()
		else:
			cur.execute("update a%s set num = num + 1 where KeyWord = '%s' and type='tool'" % (ID,tool))
			conn.commit()		
	except MySQLdb.Error,e:
